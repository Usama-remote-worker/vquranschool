...
export default function AdminStudentsPage() {
    ...
    const handleAssignTeacher = async (studentId: string, teacherId: string) => {
        ...
        try {
            const res = await fetch('/api/students/assign-teacher', {
                method: 'POST',
                                         ...
                                         <tr key={student.id} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                                             ...
                                             <td className="py-4 px-6">
                                                 <div className="flex items-center gap-2">
                                                     <select
                                                         className="text-xs border rounded-md p-1 bg-white focus:outline-none focus:ring-1 focus:ring-blue-500 disabled:opacity-50"
                                                         value={student.assigned_teacher || "none"}
                                                         onChange={(e) => handleAssignTeacher(student.id, e.target.value)}
                                                         disabled={assigningId === student.id || student.status !== 'approved'}
                                                     >
                                                     ...
                                                     {assigningId === student.id && <Loader2 className="w-3 h-3 animate-spin text-blue-600" />}
                                                 </div>
                                             </td>
